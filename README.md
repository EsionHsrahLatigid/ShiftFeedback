...
The release preset builds the standalone, VST3, and AUv2 bundles, then stages human-facing products under `artifacts/plugin-release/<platform-arch>/{standalone,vst3,au}`. For local macOS non-CI `plugin-release` builds, the staged VST3 and AU bundles are also physically copied into `~/Library/Audio/Plug-Ins/VST3` and `~/Library/Audio/Plug-Ins/Components`; the Standalone app stays under `artifacts/`. Configure with `-DEHL_COPY_PLUGIN_AFTER_BUILD=OFF` to disable the local plugin copy. macOS CI uses `macos-arm64`; Windows uses `windows-x64` without AU. `build/` remains CMake's internal workspace.
